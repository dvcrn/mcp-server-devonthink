...
const BLOCKED_PATHS = [
	...
	// User security-sensitive directories (~ prefix for home directory matching)
	"~/.ssh/",
	"~/.aws/",
	"~/.gnupg/",
	"/Library/Keychains/",
	...
	// Shell history files (~ prefix required for proper blocking)
	"~/.bash_history",
	"~/.zsh_history",
	"~/.fish_history",

	// Environment files (~ prefix for home directory matching)
	"~/.env",
	"~/.env.local",
	"~/.env.production",
];
...
export function validateFilePath(
	...
	// Expand tilde before normalization - path.resolve doesn't expand ~
	let expandedInput = filePath;
	if (filePath.startsWith("~/")) {
		expandedInput = filePath.replace("~", homeDir);
	} else if (filePath === "~") {
		expandedInput = homeDir;
	}

	// Normalize the path
	const normalizedPath = path.resolve(expandedInput);
