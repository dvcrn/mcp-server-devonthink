...
const AnalyzeDocumentThemesInputSchema = z.object({
  ...
  target: z.object({
    ...
    .optional()
    .refine(data => {
      // Target must be provided and at least one method must be specified
  ...
  const script = `
    (() => {
      ...
      try {
        ...
        // Parse input parameters (with safe access to target properties)
        ${input.target && input.target.uuid ? `const recordUuid = "${escapeStringForJXA(input.target.uuid)}";` : 'const recordUuid = null;'}
        ${input.target && input.target.recordId ? `const recordId = ${input.target.recordId};` : 'const recordId = null;'}
        ${input.target && input.target.databaseName ? `const databaseName = "${escapeStringForJXA(input.target.databaseName)}";` : 'const databaseName = null;'}
        ${input.target && input.target.recordPath ? `const recordPath = "${escapeStringForJXA(input.target.recordPath)}";` : 'const recordPath = null;'}
        ${input.target && input.target.uuids ? `const recordUuids = [${input.target.uuids.map(uuid => `"${escapeStringForJXA(uuid)}"`).join(',')}];` : 'const recordUuids = null;'}
        ...
        ${input.target && input.target.groupUuid ? `const groupUuid = "${escapeStringForJXA(input.target.groupUuid)}";` : 'const groupUuid = null;'}
            ...
            const getAllDocuments = function(group) {
              ...
              for (let i = 0; i < children.length; i++) {
                ...
                if (childType === "group" || childType === "smart group") {
                  ...
                  // Use ES5 compatible concat instead of spread operator
